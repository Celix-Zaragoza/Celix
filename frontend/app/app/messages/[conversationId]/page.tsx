...
export default function Page() {
  ...
  const conversacionActual = conversaciones.find((c) => String(c.id) === String(conversationId));
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 flex items-start gap-3 hover:bg-gray-50 transition-colors border-b ${
                    String(conversationId) === String(conversacion.id) ? "bg-blue-50" : ""
                  }`}
                >
                  <div className="relative">
                    <img src={participante.avatar} alt={participante.nombre} className="w-14 h-14 rounded-full object-cover" />
        ...
        {conversacionActual ? (
          <div className="flex-1 flex flex-col">
            ...
            <div className="p-4 border-b flex items-center gap-3">
              ...
              <img
                src={conversacionActual.participantes[0].avatar}
                alt={conversacionActual.participantes[0].nombre}
                className="w-12 h-12 rounded-full object-cover"
              />
              <div className="flex-1">
                <h3 className="font-bold text-gray-900">{conversacionActual.participantes[0].nombre}</h3>
                <p className="text-sm text-gray-600">@{conversacionActual.participantes[0].alias}</p>
              </div>
            </div>
